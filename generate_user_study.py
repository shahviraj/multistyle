...
save_path = f'/home/vjshah3/research/outputs/multistyle/userstudies/userstudy_{input_codes}_{ref_codes}/'
...
for i in range(len(ref_codes)):
    ...
    if np.random.binomial(size=1, n=1, p=0.5):
        ...
        opb = Image.open(jojogan_path + f'{ref_codes[i]}/test/{input_codes[i]}_{ref_codes[i]}.jpeg').convert('RGB')
        gt_dict[i] = {'ref_code': ref_codes[i],
                      'input_code': input_codes[i],
                      'A': 'multistyle',
                      'B': 'jojogan'}

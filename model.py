...
class EqualLinearAct(nn.Module):
    ...
    def forward(self, input):
        if self.activation == 'relu':
            ...
            raise ValueError("please check value of activation")
        return out
...
class EqualLinearActDiag(nn.Module):
    def __init__(
        self, in_dim, out_dim, init='identity', bias=True, bias_init=0, lr_mul=1, activation=None
    ):
        super().__init__()
        if init == 'identity':
            self.weight = nn.Parameter(torch.ones(in_dim).div_(lr_mul))
        elif init == 'randn':
            self.weight = nn.Parameter(torch.randn(in_dim).div_(lr_mul))
        else:
            raise NotImplementedError('this type of init is not supported yet!')
        if bias:
            self.bias = nn.Parameter(torch.zeros(out_dim).fill_(bias_init))
    ...
    def forward(self, input):
        if self.activation == 'relu':
            out = torch.dot(input, self.weight* self.scale)
            out = fused_leaky_relu(out, self.bias * self.lr_mul)
        elif self.activation == 'tanh':
            out = torch.dot(input, self.weight * self.scale) + self.bias * self.lr_mul
            out = torch.tanh(out)
        elif self.activation == None:
            out = F.linear(
                input, self.weight * self.scale, bias=self.bias * self.lr_mul
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}({self.weight.shape[0]})"
        )
